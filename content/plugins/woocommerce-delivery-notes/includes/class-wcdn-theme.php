	...
	class WCDN_Theme {
		...
		public function create_print_button_account_page( $actions, $order ) {
			if ( 'yes' === get_option( 'wcdn_print_button_on_my_account_page' ) ) {
				...
				$actions['print'] = array(
					...
					'name' => apply_filters( 'wcdn_print_button_name_on_my_account_page', __( 'Imprimir recido de inscrição', 'woocommerce-delivery-notes' ), $order ),
				);
			}
			return $actions;
		}
		...
		public function create_print_button_order_page( $order_id ) {
			...
			if ( 'yes' === get_option( 'wcdn_print_button_on_view_order_page' ) ) {
				...
				?>
				<p class="order-print">
					<a href="<?php echo esc_url( $print_url ); ?>" class="button print"><?php echo esc_attr( apply_filters( 'wcdn_print_button_name_order_page', __( 'Imprimir recido de inscrição', 'woocommerce-delivery-notes' ), $order ) ); ?></a>
				</p>
				<?php
			}
		}
