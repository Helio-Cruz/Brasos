...
if ( ! class_exists( 'Dependency_Loader' ) ) {
	...
	class Dependency_Loader {
		...
		public function load_dependencies( $class ) {
			...
			if ( 'exemple_Me' !== $parts[0] ) {
				return;
			}

			// Remove 'exemple_Me' from parts.
			array_shift( $parts );
			...
			$file_path = exemple_Me::get_plugin_path() . implode( '/', $parts ) . $class_file_name;
			...
			$file_path = exemple_Me::get_plugin_path() . implode( '/', $parts ) . $trait_file_name;
		...
		protected function load_registries() {
			require_once( exemple_Me::get_plugin_path() . 'core/registry/trait-base-registry.php' );
			require_once( exemple_Me::get_plugin_path() . 'core/registry/class-controller.php' );
			require_once( exemple_Me::get_plugin_path() . 'core/registry/class-model.php' );
		}
		...
		protected function load_core() {
			...
			foreach ( glob( exemple_Me::get_plugin_path() . 'core/*.php' ) as $path ) {
				require_once $path;
			}
		}
